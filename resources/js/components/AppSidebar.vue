<template>
    ...
    >
        <div class="flex items-center gap-3 px-5 py-5 border-b border-sidebar-border">
            ...
            <div class="min-w-0">
                <div class="text-sm font-semibold text-white leading-tight truncate">Wholesale</div>
                <div class="text-xs text-sidebar-muted leading-tight truncate">Inventory</div>
            </div>
        </div>

        <nav class="flex-1 min-h-0 overflow-y-auto px-3 py-4 space-y-1" aria-label="Primary">
            <router-link
                v-if="auth.hasRole(['admin','manager','cashier','warehouse'])"
                to="/pos"
                class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-semibold transition-colors bg-brand-600 text-white hover:bg-brand-700"
            >
            ...
            <router-link
                ...
                :class="isActive('/dashboard')
                    ...
                    : 'text-sidebar-fg hover:bg-sidebar-active hover:text-white'"
            >
                <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <template v-for="section in sections" :key="section.id">
                <div v-if="hasAnyVisibleItem(section)" class="pt-3">
                    <button
                        type="button"
                        class="w-full flex items-center justify-between px-3 py-1.5 text-xs font-semibold uppercase tracking-wider text-sidebar-muted hover:text-white transition-colors"
                    ...
                    >
                        <div class="mt-1 space-y-0.5">
                            <router-link
                                ...
                                :key="item.path"
                                :to="item.path"
                                class="flex items-center gap-3 pl-9 pr-3 py-2 rounded-lg text-sm transition-colors"
        ...
        <div class="px-3 py-3 border-t border-sidebar-border">
            <div class="flex items-center gap-3 px-3 py-2">
                <div class="w-8 h-8 rounded-full bg-brand-600 flex items-center justify-center text-white text-xs font-semibold shrink-0">
...
const sections = [
    {
        ...
        label: 'Sales',
        items: [
            { path: '/sales', label: 'Sales history', icon: ICONS.receipt, roles: ['admin', 'manager', 'cashier', 'warehouse'] },
        ],
    },
    {
        id: 'admin',
    ...
    },
    {
        ...
        label: 'Master Data',
        items: [
            { path: '/master/categories', label: 'Categories', icon: ICONS.tag, roles: null },
            { path: '/master/brands', label: 'Brands', icon: ICONS.cube, roles: null },
            { path: '/master/suppliers', label: 'Suppliers', icon: ICONS.truck, roles: ['admin', 'manager', 'purchasing'] },
            { path: '/master/customers', label: 'Customers', icon: ICONS.usersGroup, roles: null },
        ],
    },
    {
        id: 'inventory',
        label: 'Inventory',
        items: [
            { path: '/products', label: 'Products', icon: ICONS.cube, roles: null },
            { path: '/batches', label: 'Batches', icon: ICONS.inbox, roles: null },
            { path: '/units', label: 'Units', icon: ICONS.scale, roles: ['admin', 'manager'] },
            { path: '/warehouses', label: 'Warehouses', icon: ICONS.building, roles: ['admin', 'manager'] },
        ],
    },
];
